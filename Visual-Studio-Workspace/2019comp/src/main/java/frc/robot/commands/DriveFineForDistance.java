...
public class DriveFineForDistance extends Command {
  double goal;
  boolean done;
  int counter = 0;
  public DriveFineForDistance(double distance) {
    // Use requires() here to declare subsystem dependencies
    // eg. requires(chassis);
    requires(Robot.drivetrain);
    requires(Robot.stayStraightPID);
    requires(Robot.fineMovePID);
    goal = distance;
  }
  ...
  protected boolean isFinished() {
    ...
    return Robot.fineMovePID.isDone();
  }
